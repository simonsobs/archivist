...
def test_librarian_callback_config_parses_and_reads_its_token_file(tmp_path):
    """Callback destinations are keyed by librarian_name; tokens come from a file, not the config."""
    ...
    settings = Settings(
        librarians={
            ...
            "lib-b": {"url": "http://other.example/", "auth_token_file": str(token_path)},
        }
    )

    assert settings.librarians["lib-a"].url == "http://librarian.example/"
    assert settings.librarians["lib-a"].auth_token is None
