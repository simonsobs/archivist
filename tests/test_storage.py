...
def _archive_for(source, local_root, archive_root, size=1024):
    return ArchiveJob(
...
def test_disk_verify_archive(use_settings, local_root, archive_root):
    ...
    assert storage.verify(_archive_for(source, local_root, archive_root, size=7)) is False

    # Present, but truncated -- a crash mid-copy looks exactly like this.
    dest = archive_root / "file.txt"
    dest.write_text("con")
    assert storage.verify(_archive_for(source, local_root, archive_root, size=7)) is False
    dest.write_text("content")
    assert storage.verify(_archive_for(source, local_root, archive_root, size=7)) is True
