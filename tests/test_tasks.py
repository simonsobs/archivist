...
def enqueue_manifest(db_session, archive_root):
    ...
    def _enqueue(source_path, manifest_id="m1"):
        request = ManifestRequest(**make_manifest_request(instance_path=str(source_path)))
        db_session.add(
            ArchiveQueue.new_item(
...
def _drain_until_completed(session, manifest_id, timeout=5):
    ...
    while time.time() < deadline:
        ...
        session.expire_all()
        item = session.query(ArchiveQueue).filter_by(manifest_id=manifest_id).one()
        if item.completed:
            return item
        time.sleep(0.05)
...
def test_start_archive_consumes_the_row_and_copies_the_file(db_session, enqueue_manifest, local_root, archive_root):
    ...
    enqueue_manifest(source)

    start_archive(librarian_name="test-librarian")
    assert db_session.query(ArchiveQueue).filter_by(manifest_id="m1").one().consumed
...
def test_process_status_queue_marks_rows_completed_or_failed(db_session, enqueue_manifest, local_root):
    ...
    start_archive(librarian_name="test-librarian")
    with mock.patch.object(ArchiveQueue, "complete", side_effect=RuntimeError("boom")):
