...
def test_manifest_request_accepts_many_or_no_entries():
    ...
    assert ManifestRequest(librarian_name="lib", store_files=[]).store_files == []
...
def test_create_archive_posix():
    ...
    assert pairs == [
        (Path("/local/sub/file.txt"), Path("/archive/sub/file.txt")),
        (Path("/local/b.txt"), Path("/archive/b.txt")),
    ]
...
def test_create_archive_returns_none_for_unsupported_types(archive_type):
    assert Archive(manifest={"store_files": []}, type=archive_type).create_archive() is None
